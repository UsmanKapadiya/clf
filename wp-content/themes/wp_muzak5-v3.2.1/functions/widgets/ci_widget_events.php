...
class CI_Events extends WP_Widget {
	...
	function form($instance){
		$instance 	= wp_parse_args( (array) $instance, array('title'=>'', 'events_no'=>3, '' => 'show_recurrent') );
		...
		echo '<p><label for="'.$this->get_field_id('events_no').'">' . __('Events Number:', 'ci_theme') . '</label><input id="' . $this->get_field_id('events_no') . '" name="' . $this->get_field_name('events_no') . '" type="text" value="' . esc_attr($events_no) . '" class="widefat" /></p>';
 		echo '<p><input id="' . $this->get_field_id('show_recurrent') . '" name="' . $this->get_field_name('show_recurrent') . '" type="checkbox" value="on" '.checked($show_rec, "on", false).' /> <label for="'.$this->get_field_id('show_recurrent').'">' . __('Show recurrent events?', 'ci_theme') . '</label></p>';
